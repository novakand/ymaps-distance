...
export class LineLayerDirective implements OnInit, OnDestroy {
    ...
    @Input()
    set options(options: LineLayerProps<{}>) {
    ...
    private readonly _id: string = (LineLayerDirective.layerCounter++).toString();
    ...
    private _updateLayer(opts: LineLayerProps<any>): void {
        if (this.layer) {
            this._deckService.removeLayer(this);
            this.layer = undefined;
        }

        this.layer = new LineLayer(opts);
        this._deckService.addLayer(this, this.layer);
    }

    private _combineOptions(): Observable<LineLayerProps<{}>> {
        return combineLatest([this._options, this._data, this._visible]).pipe(
            map(([options, data, visible]) => {
                const combinedOptions: LineLayerProps<{}> = {
                    ...options,
    ...
    private _assertInitialized(): asserts this is { layer: LineLayerProps<{}> } {
        if (!this.layer) {
            throw new Error(
