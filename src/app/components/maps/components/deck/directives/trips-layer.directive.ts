...
@Directive({
    selector: 'trips-layer',
    standalone: true
})
export class TripsLayerDirective implements OnInit, OnDestroy {
    ...
    @Input()
    set options(options: TripsLayerProps<{}>) {
    ...
    public layer?: TripsLayer;

    private static layerCounter = 0;

    private readonly _id: string = (TripsLayerDirective.layerCounter++).toString();
        ...
        this._combineOptions()
            ...
            .subscribe(options => {
                this._ngZone.runOutsideAngular(() => {
                    this.layer = new TripsLayer(options);
    ...
    private _updateLayer(opts: TripsLayerProps<any>): void {
        if (this.layer) {
            this._deckService.removeLayer(this);
            this.layer = undefined;
        }

        this.layer = new TripsLayer(opts);
        this._deckService.addLayer(this, this.layer);
    }

    private _combineOptions(): Observable<TripsLayerProps<{}>> {
        return combineLatest([this._options, this._data, this._visible]).pipe(
            map(([options, data, visible]) => {
                const combinedOptions: TripsLayerProps<{}> = {
                    ...options,
    ...
    private _assertInitialized(): asserts this is { layer: TripsLayerProps<{}> } {
        if (!this.layer) {
            throw new Error(
